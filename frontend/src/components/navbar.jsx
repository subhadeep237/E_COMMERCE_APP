...
const Navbar = () => {
  ...
  const { cartCount, token, setToken, setCartItems } = useContext(ShopContext)
    ...
    <div style={{
      ...
      <div style={{display:'flex', gap:'20px', alignItems:'center', marginRight:'40px'}}>
        ...
        <div
          ...
          onMouseEnter={()=> {
            if(token) setDropdownVisible(true)
          }}
          onMouseLeave={()=> setDropdownVisible(false)}
        >
          <img
            src={frontend_assets.profile_icon}
            alt=''
            width='20px'
            style={{cursor:'pointer'}}
            onClick={()=> {
          ...
          {/* Dropdown only shows when logged in AND hovering */}
          {token && dropdownVisible && (
            <div style={{
              ...
              <p style={{cursor:'pointer', padding:'5px 0'}}>My Profile</p>
              <p style={{cursor:'pointer', padding:'5px 0'}} onClick={()=>navigate('/orders')}>Orders</p>
              <p style={{cursor:'pointer', padding:'5px 0'}} onClick={logout}>Log Out</p>
            </div>
          )}
        </div>
